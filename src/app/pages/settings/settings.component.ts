...
@Component({
  ...
  imports: [MatCardModule, MatIconModule, MatTreeModule],
  standalone: true,
})
export class SettingsComponent implements OnInit {
    ...
    this.getNoteBooks()
      .pipe(
        switchMap((notebooks) => {
          ...
          const sectionObservables = notebooks.map((notebook) =>
            this.getSections(notebook.id)
          );
  ...
  private getNoteBooks(): Observable<SettingTreeNode[]> {
    return this.onenoteService.getNoteBooks().pipe(
      map((booksResult) => {
  ...
  private getSections(noteBookId: string): Observable<SettingTreeNode[]> {
    return this.onenoteService.getSections(noteBookId).pipe(
      map((sectionsResult) => {
  ...
  hasChild = (_: number, _nodeData: SettingTreeNode) =>
    _nodeData.children && _nodeData.children.length > 0;
}
